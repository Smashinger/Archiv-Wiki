...
Das Hauptfenster besitzt eine eigene, in die Anwendung integrierte Titelleiste anstelle der nativen Fensterdekoration (technische Grundlage siehe `12_KNOWN_DECISIONS.md`). Sie ist bewusst kein zweites, aufgesetztes Element, sondern der natürliche obere Abschluss des gesamten Fensters:

- Derselbe Fenster-Grundhintergrund (`--bg-color`) wie der Rest der Anwendung, keine eigene, zweite Hintergrundebene.
- Kein dekorativer Schatten; Abschluss nach unten über einen hauchdünnen `--border-soft`-Rand statt einer harten Trennlinie.
- Kompakte, flache Höhe (`--titlebar-h`, 34px) — deutlich niedriger als die interne Topbar (`--topbar-h`).
- App-Symbol und Anwendungsname („Archiv Wiki“) bleiben klein und zurückhaltend (Marken-Schriftart `--font-brand`, aber ohne Überschriften-Größe) — keine große Seitenüberschrift.
- Menüeinträge (Datei/Bearbeiten/Ansicht/Hilfe) sind kompakte, textbasierte Knöpfe mit ruhigem Hover-Farbwechsel (`--overlay-medium`), keine großen Buttons.
- Fensterknöpfe (Minimieren/Maximieren/Schließen) sind monochrom und besitzen keinen dauerhaft sichtbaren Rahmen/Hintergrund — nur ein Hover-Farbwechsel, 150ms, dieselbe Übergangsdauer wie im übrigen System.
- Einzige bewusste Ausnahme vom neutralen Hover: der Schließen-Knopf färbt sich bei Hover vollflächig mit der bestehenden Fehler-/Destruktivfarbe (`--red`) — etabliertes Fenstersteuerungs-Muster, keine neue Rotfarbe.
- Die freie Fläche zwischen Menü und Fensterknöpfen dient ausschließlich dem Verschieben des Fensters; sie bekommt keine zusätzlichen Bedienelemente.
