...
## Projektidentität (Version 1.x)

### Entscheidung

Archiv-Wiki verwendet in Version 1.x den **normalisierten Projektpfad** als eindeutige Identität für projektbezogene Laufzeitdaten außerhalb des eigentlichen Wiki-Projekts, beispielsweise für den Backup-Status.

Für Version 1.x wird bewusst **keine `projectId` oder UUID** eingeführt. Der Projektpfad bleibt die einzige Identität für projektbezogene Laufzeitdaten.
...
Das Verschieben eines Wiki-Ordners wird in Version 1.x bewusst als **neue Projektinstanz für app-weite Laufzeitdaten** behandelt.
...
- Zwei getrennte Icon-Systeme: kuratierte SVG-Bibliothek (thematisch, Kategorie-/Notiz-Icons) und Unicode/Text (funktionale UI-Elemente). Keine bunten Emoji in funktionalen UI-Elementen.
...
- Drei Ansichtsmodi (Editor/Split/Vorschau), Modus und Split-Breite pro Notiz gespeichert.
...
- Der Fokus-Modus lässt die übrige Oberfläche sichtbar und bedienbar, blendet nichts vollständig aus; vier Intensitätsstufen, Intensität wird gespeichert, der An/Aus-Zustand selbst nicht.
...
- Ein zentrales Overlay, sechs Abschnitte: Allgemein, Darstellung, Editor, Backup, Updates, Sicherheit.
...
## Atomare Schreibstrategie für Primärdaten (Version 2.0)
