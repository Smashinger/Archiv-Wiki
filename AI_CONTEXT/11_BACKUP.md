...
Im Einstellungsfenster, Abschnitt "Backup" (siehe `09_SETTINGS.md`): Der Bereich öffnet mit der Zustandszeile — letztes erfolgreiches und nächstes geplantes Backup, dazu die Aktion "Jetzt sichern". Sie ist die einzige blau gefüllte Hauptaktion des ganzen Fensters und trägt bei Handlungsbedarf (noch kein Backup, kein Ordner gewählt, Zeitpunkt überfällig) eine amberfarbene linke Kante. Darunter Backup-Ordner (mit "Ordner wählen…"), Intervall-Auswahl, der Umfang eines Backups sowie eine kurze Anleitung zur manuellen Wiederherstellung mit dem Knopf "Backup-Ordner öffnen". Letzter projektbezogener Fehler und Aufräumwarnung erscheinen als Textzeile unter der Zustandszeile, nicht als eigene Kästen.
