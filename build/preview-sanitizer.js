...
// marked gibt die GFM-Spaltenausrichtung als veraltetes align-Attribut an
// jeder th-/td-Zelle aus. In der echten Vorschau geht dieses Attribut trotz
// Allowlist an der DOMPurify-Grenze verloren. Vor der Sanitization wird nur
// die exakt bekannte marked-Ausgabe auf drei feste Klassen abgebildet. Die
// Klassen selbst durchlaufen weiterhin die normale Allowlist; freie Styles
// oder beliebige Attributwerte werden dadurch nicht zugelassen.
function normalizeTableAlignments(html) {
  return String(html || '').replace(
    /<(th|td) align="(left|center|right)">/gi,
    (_match, tag, alignment) => `<${tag} class="markdown-align-${alignment.toLowerCase()}">`
  );
}
