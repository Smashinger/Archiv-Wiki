...
Drei Stufen, konsequent nach Elementgröße vergeben:
...
Keine hartkodierten, abweichenden Rundungswerte außerhalb dieser drei Stufen.
...
- **Standardwert: 150ms, `ease`.** Die weit überwiegende Mehrheit aller Übergänge in der Anwendung verwendet exakt diesen Wert. Neue Übergänge verwenden denselben Wert, keine abweichende, neu erfundene Zeit.
- Übergänge betreffen ausschließlich Farbe, Rahmen, Hintergrund — keine Positions- oder Größenanimationen mit Schwebe-Charakter.
- Schatten werden nur an einer bewusst gewählten, seltenen Stelle eingesetzt (Bild-Vergrößerungsansicht), nicht als allgemeines Stilmittel.
...
Zwei getrennte Systeme, die nicht vermischt werden:
1. **Kuratierte SVG-Icon-Bibliothek** — ausschließlich für thematische, vom Nutzer gewählte Kategorie-/Notiz-Icons.
2. **Unicode-Symbole/Text** — für funktionale UI-Elemente (z. B. Editor-Werkzeugleiste). Wo möglich zeigt das Symbol die tatsächlich ausgelöste Markdown-Syntax (`{ }` für Code, `1.` für nummerierte Listen) — das ist selbsterklärender als abstrakte Symbole.
