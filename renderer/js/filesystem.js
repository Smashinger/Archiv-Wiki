...
// D5: reine Textauflösung (kein Datei-/Notizzugriff) — dieselbe Funktion, die
// createNote() intern für {title}/{date}/{time}/{year} nutzt.
export async function resolveTemplateVariables(text, title) {
  return window.archivAPI.fs.resolveTemplateVariables(text, title);
}
...
export async function collectNotesByTags(tags) {
  return window.archivAPI.fs.collectNotesByTags(tags);
}

export async function applyTagOperation(operation, snapshot) {
  return window.archivAPI.fs.applyTagOperation(operation, snapshot);
}

export async function undoTagOperation(undoEntries) {
  return window.archivAPI.fs.undoTagOperation(undoEntries);
}

export async function collectNoteSnapshots(relPaths) {
  return window.archivAPI.fs.collectNoteSnapshots(relPaths);
}

export async function applyBatchMove(snapshot, targetRelPath) {
  return window.archivAPI.fs.applyBatchMove(snapshot, targetRelPath);
}

export async function applyBatchArchive(snapshot) {
  return window.archivAPI.fs.applyBatchArchive(snapshot);
}

export async function applyBatchTrash(snapshot) {
  return window.archivAPI.fs.applyBatchTrash(snapshot);
}

export async function undoBatchMove(undoEntries) {
  return window.archivAPI.fs.undoBatchMove(undoEntries);
}
