...
// Prüft, ob ein Dokument zu den aktiven Kategorie-/Tag-/Status-Filtern passt.
...
// Archiv-Status (Feature A / Block 3): 'active' (Standard) blendet
// archivierte Notizen aus JEDER Suche aus, auch ohne gesetzten Kategorie-/
// Tag-Filter — deshalb wird diese Funktion jetzt immer aufgerufen (siehe
// searchWithDetails() unten), nicht mehr nur wenn andere Filter aktiv sind.
function docMatchesFilters(doc, filters) {
  ...
  const archivedStatus = filters.archived || 'active';
  if (archivedStatus === 'active' && doc.archived) return false;
  if (archivedStatus === 'archived' && !doc.archived) return false;
  // 'all' (oder ein unbekannter Wert): kein Status-Filter.
  return true;
}
...
export function searchWithDetails(query, limit = 30, options = {}) {
  ...
  // Nur ein NICHT-Standard-Status (Archiv/Alle) zählt hier als "aktiver
  // Filter" im Sinne von "ohne Suchtext trotzdem Ergebnisse zeigen" — der
  // Standard 'active' ist unsichtbarer Grundzustand, kein bewusst gesetzter
  // Filter (siehe auch hasActiveFilters()/updateFilterUi() in app.js).
  const hasActiveFilters = Boolean(filters.category || (filters.tags && filters.tags.length) || (filters.archived && filters.archived !== 'active'));
  ...
  // docMatchesFilters() wird jetzt IMMER aufgerufen (nicht mehr nur wenn
  // Kategorie/Tags gesetzt sind) — der Archiv-Status-Filter mit Standard
  // 'active' muss für jede Suche gelten, auch ganz ohne andere Filter.
  const filteredRelPaths = candidateRelPaths.filter(relPath => docMatchesFilters(docsByRelPath.get(relPath), filters));
